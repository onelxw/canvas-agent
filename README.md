...
| 最近一次核对的上游版本 | `v0.18.0` / `d213a74`（2026-09-07，已选择性移植通用修复，核对日期 2026-09-11） |
...
- **MiniMax H3 视频适配**：删除原默认 `grok-imagine-video`，内置文生视频、图生视频、多图多音频三个模型；使用 `/v1/videos` JSON 协议、本地参考图自动上传、任务 ID 续查，并由模型名自动决定参考素材规则。
- **工作台参数**：生图参数改为“质量、比例、生成张数”下拉框；视频工作台和画布视频节点使用清晰度、比例下拉框与时长滑杆，比例选项显示对应分辨率，不再提供自定义宽高、清晰度或首尾帧/全能参考模式选择。
...
- **上游 v0.18.0 通用修复**：已移植 Gemini 生图 `imageConfig`、OpenAI 多参考图 `image[]`、生成提示词文本编号，以及资产从 IndexedDB 本地文件下载的修复；未合并本地代理、图片 1K/2K/4K 固定尺寸表、URL 导入多渠道等与当前定制冲突或需单独评估的功能。
- **上游 v0.18.0 功能同步**：已选择性移植节点多选打组/解散、视频任务刷新续查、遮罩标注参考图、参考视频/音频底层能力，以及 WebDAV 画布删除记录；上游的视频模式与参数界面已由本分支 MiniMax H3 专用适配替代，未引入本地代理和全屏调用脚本编辑器。
