...
const descriptions: Record<InternalAgentToolName, string> = {
    ...
    canvas_create_generation_flow: "创建生成配置流程，但不会自动开始付费生成。已有文本节点时必须传 promptNodeId 直接复用并连线，不要重复创建提示词节点；仅在没有现成提示词节点时传 prompt 创建新文本节点。referenceNodeIds 用于图片等其他参考节点；视频流程可用 videoMode=frames 或 reference 选择首尾帧或全能参考。",
