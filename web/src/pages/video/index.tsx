                    ...
                    <WorkbenchScrollArea className="rounded-lg border border-stone-200 bg-card shadow-sm dark:border-stone-800 lg:min-h-0" viewportClassName="flex flex-col p-4 lg:h-full lg:overflow-y-auto">
                        ...
                        <div className="mt-6 space-y-5">
                            ...
                            <div className="min-w-0">
                                ...
                                <div
                                    className={`hover-scrollbar flex min-h-24 w-full min-w-0 max-w-full gap-2 overflow-x-auto overflow-y-hidden rounded-lg border border-dashed p-2 pb-3 overscroll-x-contain transition-colors ${referenceDragTarget ? "border-stone-900 bg-stone-100/80 dark:border-stone-100 dark:bg-stone-900/80" : "border-stone-300 dark:border-stone-700"}`}
                            ...
                            <div className="flex items-center justify-between rounded-lg border border-stone-200 bg-stone-50 px-3 py-2 text-sm dark:border-stone-800 dark:bg-stone-900 sm:hidden">
                                <span className="truncate text-stone-500 dark:text-stone-400">
                                    {modelOptionLabel(effectiveConfig, model)} · {normalizeResolution(effectiveConfig.vquality)}p · {videoSizeLabel(effectiveConfig.size)} · {normalizeVideoSeconds(effectiveConfig.videoSeconds, model)}s
                                </span>
    ...
    return (
        <>
            ...
            <div className="col-span-2">
                <VideoSettingsPanel config={config} onConfigChange={(key, value) => updateConfig(key, value)} theme={theme} showTitle={false} className="space-y-4" variant="workbench" />
            </div>
        </>
    );
}
...
function normalizeVideoSeconds(value: string, model = "") {
    ...
    return isMinimaxH3Model(model) ? clampMinimaxH3Seconds(value) : clampVideoSeconds(value);
}
