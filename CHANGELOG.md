...
+ [修复] 画布视频设置弹层中的清晰度和比例下拉菜单提升到设置卡片上方，并避免点击下拉选项被误判为卡片外部点击。
+ [调整] 默认视频模型由 `grok-imagine-video` 替换为 MiniMax H3 文生视频、图生视频、多图多音频三个能力模型；旧浏览器配置会自动迁移并移除原默认模型。
+ [新增] MiniMax H3 使用独立 JSON 请求适配：按模型名自动决定参考素材规则，支持本地参考图自动上传、1～15 秒、480P/768P、横版/竖版，以及 10 秒间隔的任务续查。
+ [调整] 画布视频节点与视频工作台移除首尾帧/全能参考模式选择；画布节点清晰度和比例改为带实际分辨率的下拉菜单，并移除自定义清晰度与宽高输入。
+ [修复] WebDAV 同步记录已删除画布，后续同步不再把远端旧画布恢复到本地；本地代理功能未随此修复引入。
+ [新增] 多选节点时显示整体虚线选区，并支持一键打组；组节点支持从工具栏或右键菜单解散。
+ [调整] 曾同步上游的视频清晰度、比例和时长控件；当前默认 MiniMax H3 模型进一步收敛为 480P/768P、横版/竖版和 1～15 秒。
+ [新增] 保留上游视频参考素材底层能力；当前默认 MiniMax H3 由模型名自动选择文生、图生或多图多音频规则，不再手动选择首尾帧/全能参考模式。
+ [新增] 画布视频节点保存远端任务 ID，刷新后自动继续查询状态，也可手动恢复查询，直到生成成功、任务失败或查询报错。
+ [修复] 从上游 v0.18.0 移植 Gemini 生图 `generationConfig.imageConfig` 参数格式、多参考图编辑 `image[]` 字段、生成提示词文本编号及资产本地文件下载修复。
+ [文档] README 新增精确上游分叉点、分叉功能增删改清单、通用合并候选、本分支定制保护项及 AI 上游同步检查步骤。
...
+ [调整] 官方节点插件注册表切换到 `onelxw/canvas-agent` 的 `plugins-dist` 分支，并迁移保留 Markdown、SVG、HTML、3D 全景和便利贴五个现有官方插件。
+ [调整] 空白画布的节点创建菜单改为单击右键打开，双击左键不再触发；节点与连线原有右键菜单保持不变。
+ [调整] 所有标准页面顶部统一移除文档与 GitHub 图标及跳转入口；生图与视频工作台的核心生成参数改为下拉选择，比例选项展示对应分辨率并移除自定义尺寸/清晰度输入。
+ [优化] 生图与视频工作台隐藏三张主卡片的系统滚动条，改用无箭头、可拖动且会同步滚动位置的 3px 自绘滑块；滑块仅在滚动或拖动时淡入，闲置约 0.9 秒后自动隐藏；参考图区移除悬停时的模拟滚动条，仅在内容真实溢出时显示横向滚动条。
+ [调整] 主页、我的画布、生图工作台、视频创作台、提示词库、我的资产和配置页不再显示版本号、更新日志及检查更新入口。
+ [调整] 网站品牌名由“无限画布”更新为“无限次元”，顶部品牌位与浏览器页签改用随网站深浅主题切换的方形无限符号 PNG。
+ [调整] 主页顶部移除文档与 GitHub 外链入口；画布与主页均不再展示这些跳转图标。
+ [调整] 渠道配置收敛为单一内置渠道，接口地址固定为 `https://api.aiacg.de`，移除新增、删除与二级编辑入口，协议、API Key 和模型设置直接在渠道页编辑。
...
+ [安全] 配置文件默认不再导出渠道 API Key 和 WebDAV 密码；工具结果会脱敏并限制长度，模型不能调用配置、网络、脚本或任意 URL 工具。
+ [测试] 新增内置 Agent 协议解析、工具白名单、严格校验、重复调用防护、确认拒绝、revision 冲突、操作上限及上下文裁剪测试。
